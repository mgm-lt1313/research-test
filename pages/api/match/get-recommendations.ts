...
// 自分のコミュニティIDを取得するヘルパー
async function getMyCommunityId(client: PoolClient, selfId: string): Promise<number | null> {
    const res = await client.query('SELECT community_id FROM communities WHERE user_id = $1', [selfId]);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const myCommunityId = await getMyCommunityId(client, selfId);
        
        // 設計書 6.2 (マッチングスコア) と 6.3 (多様性) に基づくクエリ
        // 1. 類似度情報を取得し、コミュニティボーナスを加算
        const query = `
        ...
        const { rows } = await client.query(query, [selfId, myCommunityId]);
